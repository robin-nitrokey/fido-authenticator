...
pub struct Config {
    ...
    /// If set, the first Get Assertion request within the specified time after boot is accepted
    /// without additional user presence verification.
    ...
    T: TrussedRequirements,
{
    ...
    fn skip_up_check(&mut self) -> bool {
        ...
        if let Some(timeout) = self.config.auto_accept_timeout.take() {
            let uptime = syscall!(self.trussed.uptime()).uptime;
            if uptime < timeout {
                info_now!("auto-accept up directly after boot");
                return true;
            }
        }
        false
